...
import os, json, sqlite3, datetime, time, math, argparse, re
...
try:
    ...
    requests = None  # manter execucao sem Telegram
...
# Ex.: 0.3.7 - Histerese de alívio, soft-cap inteligente e agrupador anti-churn
VERSIONS_FILE = ""

LOOKBACK_DAYS = 7
MONTHLY_PROFIT_GOAL_PPM = None
MONTHLY_PROFIT_GOAL_SAT = None
MONTHLY_PROFIT_GOAL_SAT_7D = None
GOAL_SHORTFALL_TOL = 0.05
GOAL_SURPLUS_TOL = 0.10
...
LIMITS = {
    ...
    "TOP_REVENUE_SURGE_BUMP":  (0.05, 0.30),
...
ASSISTED_WINDOW_DAYS   = 60         # janela de validade do crédito de liquidez (ajustado)
ASSISTED_WEIGHT_ALPHA  = 1       # peso no ajuste (ajustado)
...
def _goal_ratios(profit_sat, profit_ppm):
    ratios = []
    if MONTHLY_PROFIT_GOAL_PPM:
        try:
            target = float(MONTHLY_PROFIT_GOAL_PPM)
            if target > 0:
                ratios.append((profit_ppm or 0.0) / target)
        except Exception:
            pass
    if MONTHLY_PROFIT_GOAL_SAT_7D:
        try:
            target = float(MONTHLY_PROFIT_GOAL_SAT_7D)
            if target > 0:
                ratios.append((profit_sat or 0.0) / target)
        except Exception:
            pass
    return ratios


def _goal_deviation(profit_sat, profit_ppm):
    ratios = _goal_ratios(profit_sat, profit_ppm)
    if not ratios:
        return 0.0, 0.0
    min_ratio = min(ratios)
    max_ratio = max(ratios)
    return min_ratio - 1.0, max_ratio - 1.0
...
def adjust(overrides, kpis, symptoms):
    ...
    profit_ppm = kpis.get("profit_ppm_out_adj",
                  kpis.get("profit_ppm_adj",
                  kpis.get("profit_ppm_est")))
    ...
    TOP_REVENUE_SURGE_BUMP = get("TOP_REVENUE_SURGE_BUMP")
    ...
    shortfall, surplus = _goal_deviation(prof_sat, profit_ppm)
    goal_action = None
    if shortfall < -GOAL_SHORTFALL_TOL:
        boost = min(0.05 + abs(shortfall) * 0.4, 0.15)
        goal_action = "below"

        new_surgek = clamp(SURGE_K + boost, *LIMITS["SURGE_K"])
        if _set_change(changed, "SURGE_K", new_surgek, SURGE_K):
            SURGE_K = new_surgek

        new_surge_bump = clamp(SURGE_BUMP_MAX + boost * 0.5, *LIMITS["SURGE_BUMP_MAX"])
        if _set_change(changed, "SURGE_BUMP_MAX", new_surge_bump, SURGE_BUMP_MAX):
            SURGE_BUMP_MAX = new_surge_bump

        new_top_bump = clamp(TOP_REVENUE_SURGE_BUMP + boost * 0.4, *LIMITS["TOP_REVENUE_SURGE_BUMP"])
        if _set_change(changed, "TOP_REVENUE_SURGE_BUMP", new_top_bump, TOP_REVENUE_SURGE_BUMP):
            TOP_REVENUE_SURGE_BUMP = new_top_bump

        new_out_floor = clamp(OUTRATE_FLOOR_FACTOR + boost * 0.3, *LIMITS["OUTRATE_FLOOR_FACTOR"])
        ...
        cool_step_down = min(1.0, abs(shortfall) * 6.0)
        new_cool_down = clamp(COOLDOWN_DOWN - cool_step_down, *LIMITS["COOLDOWN_HOURS_DOWN"])
        if _set_change(changed, "COOLDOWN_HOURS_DOWN", new_cool_down, COOLDOWN_DOWN):
            COOLDOWN_DOWN = new_cool_down

        cool_step_up = min(0.5, abs(shortfall) * 3.0)
        new_cool_up = clamp(COOLDOWN_UP - cool_step_up, *LIMITS["COOLDOWN_HOURS_UP"])
        if _set_change(changed, "COOLDOWN_HOURS_UP", new_cool_up, COOLDOWN_UP):
            COOLDOWN_UP = new_cool_up

    elif surplus > GOAL_SURPLUS_TOL:
        relax = min(0.04 + surplus * 0.3, 0.12)
        goal_action = "above"

        new_surgek = clamp(SURGE_K - relax, *LIMITS["SURGE_K"])
        if _set_change(changed, "SURGE_K", new_surgek, SURGE_K):
            SURGE_K = new_surgek

        new_surge_bump = clamp(SURGE_BUMP_MAX - relax * 0.5, *LIMITS["SURGE_BUMP_MAX"])
        if _set_change(changed, "SURGE_BUMP_MAX", new_surge_bump, SURGE_BUMP_MAX):
            SURGE_BUMP_MAX = new_surge_bump

        new_top_bump = clamp(TOP_REVENUE_SURGE_BUMP - relax * 0.4, *LIMITS["TOP_REVENUE_SURGE_BUMP"])
        if _set_change(changed, "TOP_REVENUE_SURGE_BUMP", new_top_bump, TOP_REVENUE_SURGE_BUMP):
            TOP_REVENUE_SURGE_BUMP = new_top_bump

        new_out_floor = clamp(OUTRATE_FLOOR_FACTOR - relax * 0.3, *LIMITS["OUTRATE_FLOOR_FACTOR"])
        if _set_change(changed, "OUTRATE_FLOOR_FACTOR", new_out_floor, OUTRATE_FLOOR_FACTOR):
            OUTRATE_FLOOR_FACTOR = new_out_floor
        ...
        cool_step_down = min(1.0, surplus * 4.0)
        new_cool_down = clamp(COOLDOWN_DOWN + cool_step_down, *LIMITS["COOLDOWN_HOURS_DOWN"])
        if _set_change(changed, "COOLDOWN_HOURS_DOWN", new_cool_down, COOLDOWN_DOWN):
            COOLDOWN_DOWN = new_cool_down

        cool_step_up = min(0.5, surplus * 2.0)
        new_cool_up = clamp(COOLDOWN_UP + cool_step_up, *LIMITS["COOLDOWN_HOURS_UP"])
        if _set_change(changed, "COOLDOWN_HOURS_UP", new_cool_up, COOLDOWN_UP):
            COOLDOWN_UP = new_cool_up

    if goal_action == "below":
        causes.append("meta_lucro_reforcar")
    elif goal_action == "above":
        causes.append("meta_lucro_aliviar")
    ...
    # *** PATCH: assisted ***
    elif symptoms.get("floor_lock", 0) >= RELIEF_HYST_FLOORLOCK_MIN and kpis.get("profit_ppm_out_adj", kpis.get("profit_ppm_adj", kpis.get("profit_ppm_est", 0))) < 0:
...
def update_bad_streak(meta, prof_sat_adj, profit_ppm_out_adj):
    # marca "bad" só quando há prejuízo em sats e P&L ppm também < 0
...
def update_good_streak(meta, prof_sat_adj, profit_ppm_out_adj):
    # marca "good" quando há P&L ppm positivo e lucro em sats suficiente
...
def update_relief_hysteresis(meta, symptoms, kpis):
    # *** PATCH: assisted ***
    cond = (symptoms.get("floor_lock", 0) >= RELIEF_HYST_FLOORLOCK_MIN and kpis.get("profit_ppm_out_adj", kpis.get("profit_ppm_est", 0)) < 0)
    if cond:
...
def build_tg_message(version_info, now_local, kpis, symptoms, proposed, meta, dry_run, cooldown_blocked, discard_reasons=None, causes=None, deferred_note=None):
    ...
    kp = (
        ...
        f"pnl_ppm_on_out≈<code>{fmt_num(kpis.get('pnl_ppm_on_out',0.0))}</code>"
    )
    ...
    if "assisted_rev7d" in kpis and "profit_sat_adj" in kpis:
        kp_assist = (
            ...
            f"profit_ppm_out_adj≈<code>{fmt_num(kpis.get('profit_ppm_out_adj',0.0))}</code>"
        )
...
def main(dry_run=False, verbose=True, force_telegram=False, no_telegram=False):
    ...
    # P&L ppm “on out” e sua versão assistida
    ...
    if meta["bad_streak"] >= REQUIRED_BAD_STREAK:
        ...
        profit_ppm_eff = kpis.get("profit_ppm_out_adj", kpis.get("profit_ppm_adj", kpis.get("profit_ppm_est", 0)))
        ...
        # === PATCH: severe fast-track no agregador ===
        severe_fast = (kpis.get("profit_ppm_out_adj", kpis.get("profit_ppm_adj", 0.0)) < -190.0) and (symptoms.get("floor_lock", 0) > 120)
        if proposed:
